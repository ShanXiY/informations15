...
def news_edit_detail():
    ...
    if request.method == "GET":
        ...
        try:
            ...
            current_app.logger.error(e)
            return jsonify(errno=RET.DBERR, errmsg="获取新闻异常")
        if not news:
...
def user_count():
    ...
    try:
        #2.1获取到本月的1号，零点钟
        month_starttime_str = "%d-%d-01"%(calender.tm_year,calender.tm_mon) #2018-10-01
    ...
    try:
        #2.1获取到本日，零点钟
        day_starttime_str = "%d-%d-%d"%(calender.tm_year,calender.tm_mon,calender.tm_mday) #2018-10-22
